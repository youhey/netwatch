...
`history` は `range=2h` / `bucket=5m` / 24 points 固定で、各 point は `level` と `alert` のみを返します。`issue_count` は現在 status の reasons 数です。`provider_status` は Status Page Monitor の軽量要約で、provider ごとの `level` と `description`、全体の `level` / `alert` / `issue_count` を返します。`unknown` はこのフェーズでは `alert: false` です。
